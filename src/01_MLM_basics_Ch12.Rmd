...
# we can allow the slope to vary by specifying (1+x|county)
...
The lmer() function only accepts predictors at the individual level, so we have converted $u_j$ to $u^{full}_i = u_{j[i]}$ (with the variable county playing the role of the indexing $j[i]$), to pull out the uranium level of the county where house $i$ is located

```{r}
srrs2.fips <- srrs2$stfips*1000 + srrs2$cntyfips
cty <- cty <- read.table ("http://www.stat.columbia.edu/~gelman/arm/examples/radon/cty.dat", header=TRUE, sep=",")
